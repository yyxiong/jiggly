...
fileWatcher.watchFiles(env.componentsHome, function(filePath) {
  ...
  if (!/\S*\/(all_templates|other_templates|templates)\/\w+.hbs$/.test(filePath)) {
    return ;
  }
...
function getRealPath(viewPath) {
  return path.join(env.viewsHome, viewPath) + (env.pageMode ? "/view.hbs" : ".hbs");
};

function getComponentViewPath(compPath) {
  return path.join(env.componentsHome, compPath, "view.hbs");
};
